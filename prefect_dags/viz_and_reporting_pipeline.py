...
class VisualizationAndReportingPipeline:
    ...
    def init_wandb(self, session_info: pl.DataFrame, path: str):
        ...
        self.wandb_config['job_type'] = session_info.get_column('Device').unique().to_list()[0]
        self.wandb_config['group'] = session_info.get_column('SessionType(s)').unique().to_list()[0]
        ...
        run.log({"session_info": wandb.Table(dataframe=session_info.select(pl.exclude('Data_Server_Hyperlink')).to_pandas())})
        if path:
    ...
    def init_local_reporting(self, session_info):
        ...
        reporting_path = os.path.join(self.local_reporting_config['reporting_path_base'], session_type, rcs_num, 'session_reports', session_num)
        ...
        local_setup(reporting_path, self.local_reporting_config)
        return reporting_path


    #@task(name="{func_name}")
    def execute_viz_func(self, func_name: str, data: pl.DataFrame, func_params: dict[str, Any] = {}):
    ...
    def run(self, data_df: pl.DataFrame, analyses: Dict[str, Any], session_info: pl.DataFrame):
        ...
        for func_name, func_config in self.config['functions'].items():
            ...
            elif data_source == "dataframe" or data_source == "data" or data_source == "raw_data" or data_source == "combinedDataTable": # If data source is the raw data
                data = data_df
            else:
